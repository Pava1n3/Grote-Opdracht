...
namespace Grote_Opdracht
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Week weekAlpha = new Week();
            Week weekBeta = new Week();
            Truck Alpha = new Truck(oM, dM, weekAlpha, 1);
            Truck Beta = new Truck(oM, dM, weekBeta, 2);
            Alpha.CreateDay();
            Beta.CreateDay();

            weekAlpha.PrintOutput();
            weekBeta.PrintOutput();


            StreamWriter sw = new StreamWriter("Solution.txt");
        }

        public void GetNeighbours(Week solution)
        {
